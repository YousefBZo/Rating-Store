            ...
            <div class="content">
                ...
                <form action="" method="post" class="form form-outline m-2" enctype="multipart/form-data">
                    ...
                    <input type="submit" value="Update Store"
                        class="mx-5 my-2 bg-primary p-3 text-white border-0 rounded" name="update_store">
                </form>
            </div>
        </div>
            ...
            if (!empty($_POST['store_name']) && !empty($_POST['store_phone']) && !empty($_POST['store_address']) && !empty($_POST['store_category'])) {
                ...
                if (isset($_FILES['store_image'])) {
                    ...
                    if (in_array($fileExtension, $acceptedExtensions)) {
                        ...
                        $select_image = "SELECT store_image FROM stores WHERE store_id = $store_id";
                        $result_image = mysqli_query($con, $select_image);
                        if ($result_image) {
                            $row_image = mysqli_fetch_assoc($result_image);
                            $existingImage = $row_image['store_image'];
                            $existingImagePath = "images/$existingImage";
                        ...
                        if (move_uploaded_file($store_image_tmp, $destination)) {
                            $crud->updateStoreInDatabase($store_id, $store_name, $store_phone, $store_address, $store_category, $uniqueImageName);
